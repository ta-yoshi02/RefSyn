...
/// 操作列から詳細な構造グラフを構築
pub fn build_detailed_structure_graph(ops: &[Op]) -> (DiGraph<DetailedNode, DetailedEdge>, HashMap<DetailedNode, NodeIndex>) {
    ...
    for op in ops {
        ...
        match &op.kind {
            OpKind::AddNode { id, is_literal, label } => {
                ...
                let node_id_idx = *node_indices.entry(node_id_node.clone()).or_insert_with(|| {
                    graph.add_node(node_id_node.clone())
                });
                graph.add_edge(op_idx, node_id_idx, DetailedEdge::ReferencesNodeId);
                
                // ノードタイプをグラフに追加
                let node_type_node = DetailedNode::NodeType(*is_literal);
                let node_type_idx = *node_indices.entry(node_type_node.clone()).or_insert_with(|| {
                    graph.add_node(node_type_node.clone())
                });
                ...
                if *is_literal {
                    ...
                    let literal_idx = *node_indices.entry(literal_node.clone()).or_insert_with(|| {
                        graph.add_node(literal_node.clone())
                    });
                    graph.add_edge(op_idx, literal_idx, DetailedEdge::UsesLiteral);
                } else {
                    let label_node = DetailedNode::Label(label.clone());
                    let label_idx = *node_indices.entry(label_node.clone()).or_insert_with(|| {
                        graph.add_node(label_node.clone())
                    });
            ...
            OpKind::AddEdge { from, to, label } => {
                ...
                let from_idx = *node_indices.entry(from_node.clone()).or_insert_with(|| {
                    graph.add_node(from_node.clone())
                });
                graph.add_edge(op_idx, from_idx, DetailedEdge::EdgeDirection("from".to_string()));
                
                // toノードIDを追加
                let to_node = DetailedNode::NodeId(to.clone());
                let to_idx = *node_indices.entry(to_node.clone()).or_insert_with(|| {
                    graph.add_node(to_node.clone())
                });
                graph.add_edge(op_idx, to_idx, DetailedEdge::EdgeDirection("to".to_string()));
            ...
            OpKind::EditEdgeReference { from, old_to, new_to, label } => {
                ...
                let old_to_idx = *node_indices.entry(old_to_node.clone()).or_insert_with(|| {
                    graph.add_node(old_to_node.clone())
                });
                graph.add_edge(op_idx, old_to_idx, DetailedEdge::EdgeDirection("old_to".to_string()));
                
                // new_toノードID
                let new_to_node = DetailedNode::NodeId(new_to.clone());
                let new_to_idx = *node_indices.entry(new_to_node.clone()).or_insert_with(|| {
                    graph.add_node(new_to_node.clone())
                });
                graph.add_edge(op_idx, new_to_idx, DetailedEdge::EdgeDirection("new_to".to_string()));
            ...
            OpKind::EditEdgeLabel { from, to, old_label, new_label } => {
                ...
                let old_label_idx = *node_indices.entry(old_label_node.clone()).or_insert_with(|| {
                    graph.add_node(old_label_node.clone())
                });
                graph.add_edge(op_idx, old_label_idx, DetailedEdge::UsesLabel);
                
                // 新しいラベル
                let new_label_node = DetailedNode::Label(new_label.clone());
                let new_label_idx = *node_indices.entry(new_label_node.clone()).or_insert_with(|| {
                    graph.add_node(new_label_node.clone())
                });
    ...
    is_isomorphic_matching(
        ...
        |node_a, node_b| {
            match (node_a, node_b) {
                ...
                (DetailedNode::NodeType(a_type), DetailedNode::NodeType(b_type)) => a_type == b_type,
                // 異なる種類のノードはマッチしない
                _ => false,
            }
        },
