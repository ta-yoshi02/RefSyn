...
fn test_append_single_operation_to_program() {
    ...
    assert!(program_opt.is_some(), "Should generate program from single append operation");
...
fn test_append_multiple_operations_pattern_extraction() {
    ...
    assert!(program_opt.is_some(), "Should extract common pattern from multiple append operations");
    let program = program_opt.unwrap();
    ...
    assert!(!holes.is_empty(), "Should extract holes for parameter differences");

    // ホールに"0"と"3"が含まれていることを確認
    let hole_values: Vec<_> = holes.values().flat_map(|v| v.iter()).collect();
    assert!(hole_values.contains(&&"0".to_string()) || hole_values.contains(&&"3".to_string()), 
...
fn verify_append_pattern_structure(program: &ast::Program) {
    ...
    assert!(has_next_assignment, "append pattern should set next property");
}
...
fn verify_prepend_pattern_structure(program: &ast::Program) {
    ...
    for stmt in &program.stmts {
        match stmt {
            ...
            ast::Stmt::Assign { lhs: ast::Lhs::ObjAccess(_, prop), expr } if prop == "val" => {
                has_val_assignment = true;
            },
            ast::Stmt::Assign { lhs: ast::Lhs::ObjAccess(_, prop), expr } if prop == "next" => {
    ...
    assert!(has_val_assignment, "prepend pattern should set val property");
...
fn verify_set_pattern_structure(program: &ast::Program, holes: &HashMap<String, Vec<String>>) {
    ...
    let hole_values: Vec<_> = holes.values().flat_map(|v| v.iter()).collect();
    let has_different_values = hole_values.contains(&&"5".to_string()) || 
                              hole_values.contains(&&"4".to_string());
...
fn verify_swap_pattern_structure(program: &ast::Program, holes: &HashMap<String, Vec<String>>) {
    ...
    if unique_values.len() >= 2 {
        println!("✓ Swap pattern captured multiple different values: {:?}", unique_values);
    } else {
...
fn test_remove_first_pattern_extraction() {
    ...
    let remove_first_1_operations = vec![
        serde_json::json!({
            "editType": "addVariable",
            "id": "main-new2",
            "value": "return"
    ...
    let remove_first_2_operations = vec![
        serde_json::json!({
            "editType": "addVariable",
            "id": "main-new3", 
            "value": "return"
...
fn verify_remove_first_pattern_structure(program: &ast::Program) {
    ...
    let has_var_decl = program.stmts.iter().any(|stmt| {
        matches!(stmt, ast::Stmt::VarDecl { .. })
    });

    assert!(has_var_decl, "removeFirst pattern should contain variable declaration for return");
...
fn verify_insert_after_pattern_structure(program: &ast::Program) {
    ...
    assert!(has_node_creation, "insertAfter pattern should create new Node");
    assert!(has_val_assignment, "insertAfter pattern should set val property");
    assert!(has_next_assignment, "insertAfter pattern should modify next pointers");
}
...
fn test_concat_pattern_extraction() {
    ...
    let concat_list_operations = vec![
        serde_json::json!({
            "editType": "addEdge",
            "from": "main-new3",
            "label": "next",
...
fn verify_concat_pattern_structure(program: &ast::Program) {
    ...
    assert!(has_next_assignment, "concat pattern should connect lists via next property");
...
fn test_different_methods_produce_different_patterns() {
    ...
    if let (Some(append_program), Some(prepend_program)) = (append_result.0, prepend_result.0) {
        ...
        println!("Prepend pattern: {} statements", prepend_program.stmts.len());

        // 構造的な違いを確認
        let append_str = format!("{}", append_program);
        let prepend_str = format!("{}", prepend_program);
        
        assert_ne!(append_str, prepend_str, "Append and prepend should produce different patterns");
