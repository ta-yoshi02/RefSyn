...
fn verify_append_structure_from_analysys(program: &ast::Program, holes: &HashMap<String, Vec<String>>) {
    ...
    for stmt in &program.stmts {
        match stmt {
            // 1. Node作成の確認
            ast::Stmt::VarDecl { name, expr: ast::Expr::New(class) } => {
            ...
            // 2. val プロパティ代入の確認
            ast::Stmt::Assign { lhs: ast::Lhs::ObjAccess(obj, prop), expr } if prop == "val" => {
            ...
            // 3. next プロパティ代入の確認
            ast::Stmt::Assign { lhs: ast::Lhs::ObjAccess(obj, prop), expr } if prop == "next" => {
    ...
    // 基本構造の確認
    assert!(node_creation_count >= 1, "Should create at least 1 Node (temp1)");
    assert!(val_assignment_count >= 1, "Should assign val property (temp1.val = temp2)");
    assert!(next_assignment_count >= 1, "Should assign next property (g().next = temp1)");
    
    // ホールの確認（analysys.texの異なる値：0と3）
    assert!(!holes.is_empty(), "Should have holes for parameterized values");
    let hole_values: Vec<_> = holes.values().flat_map(|v| v.iter()).collect();
    assert!(hole_values.contains(&&"0".to_string()), "Should contain parameter value '0'");
    assert!(hole_values.contains(&&"3".to_string()), "Should contain parameter value '3'");
...
fn test_program_execution_simulation() {
    ...
    assert!(program_opt.is_some(), "Should synthesize append pattern from multiple method calls");
    let program = program_opt.unwrap();
    
    // ホールが正しく抽出されていることを確認（複数のメソッド呼び出しの違いがパラメータ化される）
    assert!(!holes.is_empty(), "Should extract holes from differences between method calls");
    
    // ホールに期待される値が含まれているかを確認
    let hole_values: Vec<_> = holes.values().flat_map(|v| v.iter()).collect();
    assert!(hole_values.contains(&&"42".to_string()) || hole_values.contains(&&"99".to_string()), 
    ...
    println!("Execution result from pattern synthesis: {:?}", execution_result);
    println!("Holes extracted from multiple calls: {:?}", holes);
    
    assert!(execution_result.created_nodes >= 1, "Synthesized pattern should create at least 1 node, got {}", execution_result.created_nodes);
    assert!(execution_result.property_assignments >= 1, "Synthesized pattern should have at least 1 property assignment, got {}", execution_result.property_assignments);
    assert!(execution_result.maintains_list_structure, "Synthesized pattern should maintain list structure");
...
fn simulate_program_execution(program: &ast::Program) -> ExecutionResult {
    ...
    for stmt in &program.stmts {
        match stmt {
            ast::Stmt::VarDecl { expr: ast::Expr::New(_), .. } => {
                created_nodes += 1;
            },
            ast::Stmt::Assign { lhs: ast::Lhs::ObjAccess(_, _), .. } => {
...
fn verify_synthesized_pattern_correctness(program: &ast::Program, holes: &HashMap<String, Vec<String>>) {
    println!("=== Pattern Synthesis Verification ===");
    
    // 1. プログラム構造の基本的な検証
    assert!(!program.stmts.is_empty(), "Synthesized pattern should have statements");
    
    // 2. ホールの存在確認（複数のメソッド呼び出し間の違いがパラメータ化されている）
    assert!(!holes.is_empty(), "Should have holes representing differences between method calls");
    ...
    for stmt in &program.stmts {
        match stmt {
            ...
            ast::Stmt::Assign { lhs: ast::Lhs::ObjAccess(_, prop), .. } if prop == "val" => {
                sets_val_property = true;
            },
            ast::Stmt::Assign { lhs: ast::Lhs::ObjAccess(_, prop), .. } if prop == "next" => {
    ...
    assert!(sets_val_property, "Synthesized pattern should set val property");
    assert!(sets_next_property, "Synthesized pattern should set next property");
    ...
    assert!(unique_values.len() >= 2, "Should have at least 2 different values from multiple method calls, got: {:?}", unique_values);
