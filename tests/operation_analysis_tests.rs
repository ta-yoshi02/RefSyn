use refsyn::operation_analyzer::{analyze_operations_with_environments, display_analysis_result, test_operation_analysis};
...
fn test_operation_analysis_detailed() {
    ...
    let result = analyze_operations_with_environments(&vis_graph, &operations_a, &operations_b).unwrap();
    
    println!("\n=== 詳細テストケースの結果 ===");
    display_analysis_result(&result);
    
    // 期待される結果の確認
    assert!(result.difference_points.len() >= 2, "少なくとも2つの差異点があるはずです");
...
fn test_operation_analysis_identical() {
    ...
    let result = analyze_operations_with_environments(&vis_graph, &operations, &operations).unwrap();
    
    println!("\n=== 同一操作列のテスト結果 ===");
    display_analysis_result(&result);
    
    // 差異がないことを確認
    assert_eq!(result.difference_points.len(), 0, "同一の操作列なので差異はないはずです");
