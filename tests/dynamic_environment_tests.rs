...
mod tests {
    ...
    fn test_dynamic_append_with_actual_operations() {
        ...
        assert!(
            program_opt.is_some(),
            "Should generate program from append operation"
        );
    ...
    fn test_multiple_method_calls_with_different_receivers() {
        ...
        let (program1_opt, _holes1) =
            find_common_pattern_from_operations(&operations_list1, &memo_envs1);

        let operations_list2 = vec![list2_prepend_operations];
        let memo_envs2 = vec![env2];
        let (program2_opt, _holes2) =
            find_common_pattern_from_operations(&operations_list2, &memo_envs2);

        // 結果を検証
        assert!(
            program1_opt.is_some(),
            "Should generate program for list1.append"
        );
        assert!(
            program2_opt.is_some(),
            "Should generate program for list2.prepend"
        );
        ...
        // プログラムが生成されていることを確認
        assert!(
            !program1.stmts.is_empty(),
            "Program1 should have statements"
        );
        assert!(
            !program2.stmts.is_empty(),
            "Program2 should have statements"
        );
    }
